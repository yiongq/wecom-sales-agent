...
- 补审（2026-09-26）：用量恢复后补跑了多角度审查，确认 15 条、驳回 13 条。
  - 1 条是缺陷：`overseas` 原为可选。表单上没勾的复选框显示成「否」，存下来却是缺，引擎于是退回看 tags 里的「国内」，漏标的国内线路会被当成境外推荐，而这个字段上架后锁定。已改为必填（spec 只要求声明它，没定可选与否；现有 20 条都写了），新建表单里必填的布尔初值给 `false`。
  - 其余 14 条是测试漏洞，都已补上断言：数值的下限与整数、各数组至少一项、空串、强度档位、`id` 正则的末尾锚与 64 位上限、锁定表逐项含顺序、「国内」按整项比而不按子串、新增键按新对象里的顺序追加、酒店条目本身冻结。对补上的断言做了 23 个变异，全部变红。
  - 「已冻结的父对象也往下走」经审查确认没问题。
