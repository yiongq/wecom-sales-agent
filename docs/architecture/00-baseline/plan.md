...
### 第 9、10 步（2026-09-26，分支 `docs/00-acceptance`）

- 复验在 `dev` 的 `d2dcf2d`（第 1–8 步全部合入）上进行：6 个 agent 各在自己的一次性 clone 里复现证据，不连服务器；最后由一个完整性审查 agent 找漏验的条目和证据不足之处。结果见「验收记录 · 第 9 步统一复验」。
- 完整性审查提出的问题都处理了：
  - 不变量 1 的「× 0.95 之后取整」用一条测试专用线路补测；不变量 27 补上 `wecom:` 以外的前缀和格式（`f28276a`）。
  - spec「不变量」一节把「本阶段补 / 本阶段新增」改成指向具体的测试，35–39 补上箭头，顶部 Revisions 行已记。
  - 1c 在 GitHub 上实际跑出一次红灯：draft PR #11 在 CI 里让 `pnpm test` 故意失败（run 36209594743），确认后关闭，临时分支已删。
- 00 结束时的前缀哈希（`d2dcf2d` 与本分支）：`PREFIX sha256 system=6c202d633b603a0b391634bcaf75da9d3ed42c30f848ad092a2467f713d9a423 tools=64c16fc8f464d5757f02411b7f8a2a6ce6f43da63416283851a6e997819692d1`。它与 00 开始前（`a198b74`）、lint 修复后（`9fec91c`）、格式化后（`292dc6d`）三次记下的值都相同。
- 本分支最终的断言数：wecom 461，dejargon 333，price-guard 403，server 269，eval mock 19/19（43 项）。
- 第 10 步核对：
  - 服务器：第 7 步的旁路实例（容器、镜像、`/opt/wecom-drill`）已删，端口 3299 空闲。
  - 本机：一次性 clone、演练 tag（`drill-*`、`zz-local-test`）、`wip/*` 分支、临时 worktree 都已删，scratchpad 只留 gitleaks 二进制；没有残留进程。
  - 保留的东西：服务器 `.env` 的备份 `/opt/wecom-sales-agent.env.bak-20260926`；远端已合并的功能分支（删不删由 owner 定）。

## 验收记录

（对照验收标准逐条验证时填写，按子编号：编号 · 通过 / 未通过 · 证据）

### 第 9 步统一复验（2026-09-26，`dev` 的 `d2dcf2d`）

54 条，48 条当场复现通过；12b、12c、12d、12f 没有重跑（要动服务器，依据第 7 步的演练记录，部署相关文件自 `demo-v1.1` 起没有改过）；6d、11e 的后半要 owner 手动做。没有失败项。

- 1a–1e · 通过：
  - 1a、1b：在干净 clone 里实测 pre-commit 与 commit-msg 拦截。
  - 1c：本地逐个门禁注入失败；GitHub 上 PR #11 实测红灯（run 36209594743）。
  - 1d：在 `git archive` 目录里跑 `pnpm install` 和四个门禁，全绿。
  - 1e：本机 `.env` 写成 prod 加一组 `FLAG_*` 前后，54 行结果摘要相同。
- 2a–2c · 通过：
  - 2a：`292dc6d` 是 `dev` 的祖先。
  - 2b：用忽略表跑 blame，改过的行大多指回更早的提交；全仓仍有 85 行指回 `292dc6d`，是 git 对拆行、合行无法对应的限制。
  - 2c：在 `292dc6d^` 上重跑 oxfmt，能逐字节复现 `292dc6d`。
- 3a · 通过（附注）：
  - pre-00 的断言和 eval 用例全部原样通过；`eval/cases.json` 与 `demo-v1` 相同。
  - 除 u2b 的客户原话外，还有两条原有断言在第 1 步的 lint 修复（`9fec91c`）里改了写法：`engine.selftest.ts` 的 `/^r-tibet/.test(x.id)` 换成 `x.id.startsWith('r-tibet')`。语义相同、断言文字不变，但严格说这是 u2b 之外对原有断言文本的改动（见 Open）。
- 3b–3d · 通过：
  - 3b：web 一半走真实 HTTP 路由；企微一半在引擎层（`handleMessage(…, 'wecom')`、E6）验证，加上 eval `robust-03-reset`。
  - 3c、3d：demo 下保鲜、模拟器、模拟支付、匿名只读都照旧。
- 3e · 通过：
  - 逐条审计 `demo-v1..d2dcf2d` 里客户能看到的改动，每一处都归入 AI 显式标识或缺陷修复 1–6，没有意外的变化。
  - 格式化提交能逐字节复现；`data/`、`pay.html`、`proposal.html`、`guide.html`、运行时依赖都没有变。
- 4a–4f · 通过：
  - 4a：E6p 覆盖 prod，以及 demo 加 `FLAG_RESET_COMMAND=off`。
  - 4b：接口实测；页面部分用无头浏览器重跑：未登录列表为空，登录后看到全部会话，退出后回到空列表。
  - 4c：带凭据付款后推送跟进，企微一侧在沙箱里没有凭据，只验到「推送失败备注」，送达用模拟器渠道和桩适配器验证。
  - 4d–4f：真实的 prod 进程实测。
- 5a、5b · 通过：真实的 `server.ts` 进程，五种错误配置都以 1 退出、只打一行原因；正常启动打一行生效值。
- 6a–6c · 通过：两段欢迎语的第一句、人工入口，以及已转人工的客户两条进入路径都不发欢迎语。6d · 通过（改名经 API 完成，见上）。
- 7a–7c、8a–8b、9a–9c · 通过：
  - 各自的测试单进程跑绿，断言文字与验收条文对得上。
  - 8a：「缺陷修复」2 表里的每一句都单独过了 `isHandoffIntent`。
  - 9a：error 日志点名了渠道。
- 10a–10c · 通过：
  - 10a：多轮、两个渠道、两种 profile 下都逐字节相同。
  - 10b：三次的值相同，见实施记录。
  - 10c：改 `sop.md` 的一个字，测试照过，只有 system 哈希变了。
- 11a–11d · 通过：
  - 11a、11b：路径黑名单与内容黑名单的正反用例全部符合。
  - 11c：假令牌在同版本 gitleaks、同一条命令下以 1 退出，输出只有 REDACTED。
  - 11d：全历史带 `-m` 扫描，没有发现泄露。
- 11e · 部分通过：两个开关已开；私有测试仓库的推送拦截没有做，要 owner 手动。
- 12a、12e · 通过：
  - 12a：用桩的 ssh、rsync 证明各种拒绝在联网之前就退出。
  - 12e：外网 `/healthz` 的 `revision` 是 `demo-v1.1`。
- 12b、12c、12d、12f · 未重跑：第 7 步已在真实服务器上演练通过（见上）；`deploy.sh`、`Dockerfile`、`server.ts` 自 `demo-v1.1` 起逐字节未变，对应的代码路径仍在。
- 13a–13f · 通过：
  - 13a：计数与 `eval/cases.json` 一致。
  - 13d：依据 `dev` 上 CI 的 lint 这一步，当时 secret 词表已注入。
  - 13f：`git grep` 核对，demo 域名只出现在 README「在线体验」一节。
- 14 · 通过：
  - spec 标注的 21 条不变量都有具体测试，36 个变异都被抓住。
  - 完整性审查指出的两处缺口已补：「× 0.95 之后取整」、其他前缀返回 400。
  - spec 的箭头已改成指向这些测试。
...
- 6d · 通过（2026-09-26）· 客服账号名从「云途定制旅行工作室客服」改成「云途定制旅行 AI 旅行顾问」。这个账号由自建应用托管，企微管理后台不能改名（提示「由自建应用托管，无法修改头像和名称」），所以在线上容器里用应用凭据调 `kf/account/update`，调用前后各用 `kf/account/list` 核对，返回 `0 ok`。以后接新客户时，只要账号由自建应用托管，也得这样改。
...
- 仍挂在 owner 名下的手动项（不影响 implemented）：私有测试仓库验证推送被拒（11e 的后半，可选）。6d 已于 2026-09-26 经 API 完成（见验收记录）。
