...
| 留意（少用） | 赭石 | 产品库有未保存的改动；字数到额度的 80%–100%；上架前的建议项没做；逐日行程条数和天数对不上；已转人工、客户在等但不到 10 分钟；系统在自动重试（缓存或索引延迟）；回滚前的「固定规则变过」提示 |
...
- 三档颜色：< 80% 中性；80%–100% 赭石；> 100% 朱砂，并写「超出 38 字，发布会被拦下」。超限不拦输入 [64]。字数上限的做法可参照 Fin：它的每条指引上限 2,500 字符 [60]。
