...
// ---------------- 该替换的：孤立嵌在中文里的商务英文 ----------------
eq('availability', dejargon('我按日期帮您确认 availability 和准确报价。', S), '我按日期帮您确认 档期 和准确报价。');
...
eq(
  '库里还有一条',
  dejargon('另外库里还有一条稻城亚丁8日的，但海拔偏高，带小朋友我就先不主推了。', S),
...
// 同样要防误伤：正常的推荐/报价话术里也会出现天数和「调整」二字。
{
  ...
  // 按句保留：摘掉承诺及其后续；剩下的若是编出来的逐日行程，整段都不要
  {
    ...
    eq('编造的逐日行程整段丢掉', keptBesideCustomPromise('没问题，给您出一个5天版。\nD1 抵达丽江\nD2 玉龙雪山\nD3 返程'), '');
  ...
  for (const t of [
    '好的，方案我先不发您了，您慢慢考虑～',
    '好的，那方案书就先不给您发了，您有需要随时说～',
    '这个我让资深顾问来帮您，稍后他会把定制方案发您。',
    '住的都是五星精选酒店。要不要我把详细方案发您看看？',
    // 光秃秃的「链接里」是在答已经发过的链接
  ...
  for (const t of [
    '海拔很高（此处海拔 3000 米以上），带老人要注意',
    '报价如下：\n\n· 每人 16,800',
    '行程亮点：\n· 熊猫基地',
    '方案书在这 /proposal/r-guizhou/2（链接里有逐日行程）',
    '（含早餐）每晚 2,800',
    '亮点：\n\n\n\n· 洱海骑行',
    // 括号里提到链接/系统生成，但写的不是链接本身
    '门票预约（详见官网链接）我们都会帮您处理',
    '订单信息（系统自动生成，请核对）：2位',
    '（此处附近有温泉）',
    '【行程】\nD1 丽江',
  ]) {
...
// 误伤一次就道歉 + 永久转人工，AI 对这个客户彻底闭嘴，所以两侧都要钉住
{
  ...
  const 必须转 = [
    ...
    '被骗了没人管',
    '这不是欺骗消费者吗',
    // 要人的几种短说法：欢迎语就教客户回「人工」，单独发这一句只可能是要人
    '人工',
    '人工！',
    '找人工',
    '人工服务',
  ...
  const 不该转 = [
    ...
    '你们有没有被投诉过',
    '我没有投诉，就问问',
    // 身份问题走身份兜底，问服务角色不等于要人；旅行话题里的「人工」也不是要人
    '你是真人还是机器人？',
    '你是真人吗',
    '你是人工智能吗',
    '有真人导游吗',
...
// 银发替代、转人工这几段是后加的长提示，逐条过一遍，防以后改提示时顺手写回「库里 / 系统」
{
  ...
  const detail = JSON.parse(
    await executeTool('get_route_detail', { routeId: 'r-sichuan-mid' }, session as never, { elder: true }),
  ) as Record<string, unknown>;
  const hints = [
    HANDOFF_NOTE,
    detail.detailNote,
    detail.intensityNote,
    detail.altitudeNote,
  ...
  eq(
    '新加的几类提示都在（长辈不适配 / 怕高反 / 体力 / 去过 / 细节口径）',
    String(
      ['不在带长辈的适配范围内', '客户担心高反', '体力强度', '去过了', '行程里没写，我让顾问确认'].every((k) =>
        hints.some((h) => h.includes(k)),
      ),
    ),
    'true',
  );
