...
const searchCall = (destination: string, ...ids: string[]) => ({
  name: 'search_routes',
  args: { destination },
  result: JSON.stringify(ids.map((id) => ({ id }))),
});
/** 本会话已经查到过这几条线（产品库的精确价只按本会话出现过的线路放行，见 price-guard routesInPlay） */
const shown = (...ids: string[]): Session =>
  blank({
...
// ---------------- 放得过：真实报价与合法档位话术 ----------------
{
  ...
  for (const t of [
    '这条线每人 12,000 元，含一晚林芝',
    '升级豪华线每人多 3,800 元，每晚都住松赞',
    '房价和团费一起算，每人 3,000 元',
    '比经济线每人贵两千多块，每晚都能住景观房',
  ])
...
// 客户说「每人两万」、模型复述「每人 20,000 元」同理——客户基本都用中文数字说预算。
{
  ...
  check(
    '几轮前用中文数字说的预算也认',
    !blocked('给您控制在每人 20,000 元以内', blank({ messages: [{ role: 'customer', content: '每人两万吧', at: 0 }] }), '好的'),
  );
  ...
  check(
    '客户说「一万九千八」读作 19800',
    parseCnAmounts('预算一万九千八').includes(19800),
    JSON.stringify(parseCnAmounts('预算一万九千八')),
  );
  ...
  check(
    '客户说「五六千」，复述「五千到六千」放行',
    !blocked(
      '每人五千到六千的预算，目前最便宜的西安线是每人 12,800 元',
      blank({ messages: [{ role: 'customer', content: '预算每人五六千', at: 0 }] }),
    ),
  );
  check(
    '客户说「八九千」，复述「八到九千」放行',
  ...
  check('客户说「8000-9000元」，复述两个端点放行', !blocked('8,000 元到 9,000 元的预算我记下了', blank(), '预算8000-9000元'));
  const { parseRangeEndpoints } = __priceGuardTest;
  check(
    '「两三万」展开成两个端点',
    JSON.stringify(parseRangeEndpoints('预算两三万')) === '[20000,30000]',
    JSON.stringify(parseRangeEndpoints('预算两三万')),
  );
  check(
    '「一两万字」「2026-10-15」不是金额区间',
    parseRangeEndpoints('攻略有一两万字，2026-10-15 出发').length === 0,
...
// 模型换一种写法就能把编的价发给客户，等于没有护栏。
{
  ...
  check(
    '「两千多块」读作 2000~3000 的区间',
    twoK.length === 1 && twoK[0].value - twoK[0].tol === 2000 && twoK[0].value + twoK[0].tol === 3000,
    JSON.stringify(twoK),
  );
  ...
  check(
    '「人均 3 万 8 左右起」读作 38000±500',
    spaced.length === 1 && spaced[0].value === 38000 && spaced[0].tol === 500,
  ...
  check(
    '全角逗号不当千分位（¥12,800，2 位…）',
    !blocked(
      `每人 ¥${cheapest.priceFrom.toLocaleString('zh-CN')}，2 位共 ¥${(cheapest.priceFrom * 2).toLocaleString('zh-CN')}`,
      shown(cheapest.id),
    ),
  );
  ...
  check(`工具报价写成中文数字放行（${cn(q.perPerson)} / ${cn(q.total)}）`, !blocked(`每人${cn(q.perPerson)}，两位一共${cn(q.total)}`, s));
  check('客户说过的中文数字预算，模型用中文复述放行', !blocked('每人两万五的预算，这两条都能覆盖', blank(), '预算每人两万五'));
  const sg = blank();
...
// 「起价」隔在人均和数字中间，护栏压根没把它当成钱——编造的线路配真实价格原样发给了客户。
{
  ...
  check(
    'P1 编造样本（本会话从没出现过松赞线）被拦',
    JSON.stringify(findUnbackedPrices(FAKE, antarctic, '想去南极')) === '[68800]',
  ...
  check(
    'P1 本轮预取召回了松赞线，同一句编造照样被拦',
    JSON.stringify(findUnbackedPrices(FAKE, antarctic, '想去南极', antarcticRecall)) === '[68800]',
    JSON.stringify(findUnbackedPrices(FAKE, antarctic, '想去南极', antarcticRecall)),
  );
  check(
    'P1 上一轮展示过松赞线，同一句编造照样被拦',
    blocked(FAKE, { ...antarctic, lastShownRoutes: shown('r-tibet-lux').lastShownRoutes }, '想去南极'),
  );
  const namedLux = '南极暂时没有现成线路，最接近的是西藏松赞全线，人均起价 68800 起';
  check(
    'P1 回复在同一句点名「松赞」线、本轮召回里有它：放行',
    findUnbackedPrices(namedLux, antarctic, '想去南极', antarcticRecall).length === 0,
  );
  check(
    'P1 回复在同一句点名「松赞」线、本会话却从没出现过：被拦（跟南极同一分句的点名不算出处）',
    blocked(namedLux, antarctic, '想去南极'),
  );
  check(
    'P1 实测的正常替代推荐（线路名和价写在一起）放行',
    findUnbackedPrices('南极暂时没有现成线路，最接近的是北欧芬兰极光玻璃屋 8 日，人均 46,800 起', antarctic, '想去南极', antarcticRecall)
      .length === 0,
  );
  ...
  check(
    'P3 最近报价是这条线（不带金额）：放行',
    !blocked('这条线人均 ¥68,800', blank({ lastQuote: { routeId: 'r-tibet-lux', routeTitle: tibetLux.title, travelers: 2 } })),
  );
  check(
    'P3 前几轮点过名：放行',
    !blocked(
      ...
      blank({
        messages: [
          { role: 'agent', content: '西藏这边推荐拉萨林芝纳木错·松赞全线 9 日', at: 0 },
          { role: 'customer', content: '那条多少钱', at: 1 },
        ],
      }),
    ),
  );
  ...
  check(
    'P3 「冰川」「极光」「雪山」「深度」点不到任何线',
    namedRoutes('冰川 极光 雪山 深度 秘境 古城', names).length === 0,
    JSON.stringify(namedRoutes('冰川 极光 雪山 深度 秘境 古城', names)),
  );
  check(
    'P3 「松赞」点到两条松赞线',
    JSON.stringify(namedRoutes('松赞', names).sort()) === '["r-tibet-lux","r-yunnan-lux"]',
  ...
  check(
    'P5 「就按这条走」不算成交语境，复述客户预算放行',
    !blocked(
      '新疆伊犁·独库公路越野摄影 9 日，起价每人 19800 元，比您说的 1 万 1 超了不少。\n一是预算能往上提一点就按这条走；二是我帮您找更实惠的。',
      xj,
      '新疆玩一周左右，每人一万一以内',
    ),
  );
  ...
  const S05 =
    '新疆一周左右，这个预算我这边有一条很对路的线，但先跟您说清楚价格情况：\n\n新疆伊犁·独库公路越野摄影 9 日\n' +
  ...
  check(
    'P5 语料原句「就按这条走；……拉回 1 万 5 以内」放行',
    !blocked(S05, s05, '新疆玩一周左右，每人一万五以内'),
    JSON.stringify(findUnbackedPrices(S05, s05, '新疆玩一周左右，每人一万五以内')),
  );
  // 同一分句里「就按这条走」接着报客户开的价，就是顺着客户砍价成交（HEAD 拦，上一版放行了）
  const yn = (said: string) =>
    blank({ lastShownRoutes: shown('r-yunnan-mid').lastShownRoutes, messages: [{ role: 'customer', content: said, at: 0 }] });
  check(
    'P5 「就按这条走，每人 6,800 元」拿客户开的价成交：被拦',
    JSON.stringify(
      findUnbackedPrices('好的，就按这条走，每人 6,800 元', yn('别家6800，你给我6800我立刻订'), '别家6800，你给我6800我立刻订'),
    ) === '[6800]',
  );
  check(
    'P5 「就按这个方案，每人 6,800 元」同样被拦',
  ...
  const tb = blank({
    lastShownRoutes: shown('r-tibet-mid', 'r-tibet-lux').lastShownRoutes,
    messages: [{ role: 'customer', content: '便宜点，11111 卖不卖', at: 0 }],
  });
  check(
    'P5 客户不带单位的数，模型截断复述放行',
    !blocked('这个价真做不了。两位如果预算定在一万一，我帮您看看贵州方向。', tb, '便宜点，11111 卖不卖'),
  );
  const tb2 = blank({ lastShownRoutes: tb.lastShownRoutes, messages: [{ role: 'customer', content: '便宜点，19999 卖不卖', at: 0 }] });
  check(
    'P5 客户 19999，模型口语复述「一万九」放行（按千位截断）',
    !blocked('两位如果预算定在一万九，我帮您看看贵州方向。', tb2, '便宜点，19999 卖不卖'),
  );
  ...
  check(
    'P5 客户说 12999，模型报「每人 13,000 元左右」：被拦',
...
// ---------------- 复核补的几处：出现过的线路怎么认 ----------------
{
  ...
  check(
    'P6 被挤出 lastShown、之前回复写过价（缩写「西藏 9 日奢华线」）：放行',
    !blocked(
      '西藏 9 日那条人均 68,800 元',
      blank({
        lastShownRoutes: evicted,
        messages: [agent('西藏两条：\n· 西藏 9 日奢华线，人均 68,800 起\n· 西藏 7 日经典线，人均 26,800 起')],
      }),
    ),
  );
  check(
    'P6 被挤出、之前写成「大理丽江」（名字倒过来）：放行',
    !blocked(
      '大理丽江那条人均 16,800',
      blank({
  ...
  check(
    'P6 顾问接管时手写的介绍（没照标题写）、交还后 AI 复述：放行',
    !blocked(
      '顾问说的那条南北疆纵贯线人均 52,800',
      blank({ messages: [agent('您好我是顾问小王，给您推荐南北疆纵贯 10 日：喀什、帕米尔、禾木、喀纳斯，人均 52,800')] }),
    ),
  );
  check(
    'P6 seenRouteIds 里有、lastShown 里已经没有：放行',
    !blocked(
      '日本那条人均 32,800 元',
      blank({
        lastShownRoutes: shown('r-bali', 'r-maldives-mid', 'r-maldives-lite', 'r-sanya', 'r-aurora').lastShownRoutes,
        seenRouteIds: ['r-japan'],
      }),
    ),
  );
  ...
  check(
    'P6 （前提）日本线已被挤出 lastShownRoutes',
    !(s.lastShownRoutes ?? []).some((r) => r.id === 'r-japan'),
    JSON.stringify(s.lastShownRoutes?.map((r) => r.id)),
  );
...
//   C05 7 月 43,560 → 10 月 39,600，「比 7 月旺季省下 3960 元」；A03「人均 2.88 万到 5.28 万三档」读成 55,800
{
  ...
  check(
    'Q2 A03 原句放行',
    !blocked(A03, shown('r-maldives-lite', 'r-maldives', 'r-bali')),
    JSON.stringify(findUnbackedPrices(A03, shown('r-maldives-lite', 'r-maldives', 'r-bali'), '')),
  );
}
...
// ---------------- 命中后只删那一句（dropSentences）/ 错价发没发出去过（saidBefore） ----------------
{
  ...
  check(
    'D1 删分点那一行，不留空分点、不留多余空行',
    drop('按 10 月 8 日出发重新算好了：\n\n· 每人 19800 元\n· 比 7 月旺季省下 3960 元\n· 为起价\n\n您看行吗？', /3960/) ===
      '按 10 月 8 日出发重新算好了：\n\n· 每人 19800 元\n· 为起价\n\n您看行吗？',
  );
  const fake = '南极没有现成线路。我们有一条南极概念的替代线路：\n\n· 南极深度体验线，人均起价 68800 起\n\n您几位出行？';
  check(
    'D2 引出语底下的分点删光了，引出语一并删',
    drop(fake, /68800/) === '南极没有现成线路。\n\n您几位出行？',
    JSON.stringify(drop(fake, /68800/)),
  );
  check(
    'D2 引出语底下还有别的分点，引出语留着',
    drop('报价如下：\n· 每人 15800 元\n· 比元旦省 5,200 元\n您看呢？', /5,200/) === '报价如下：\n· 每人 15800 元\n您看呢？',
  );
  check(
    'D3 删的是行尾那句：下一行不接上来',
    drop('每人 15,800 元。比 11 月省 5,200 元\n这是起价', /5,200/) === '每人 15,800 元。\n这是起价',
  );
  check(
    'D3 句末波浪号断句，区间里的波浪号不断',
    drop('好的～这条人均 2~3 万，比元旦省 5,200 元～您看呢', /5,200/) === '好的～您看呢',
    JSON.stringify(drop('好的～这条人均 2~3 万，比元旦省 5,200 元～您看呢', /5,200/)),
  );
  check(
    'D4 换成一句话时保留分点符',
    drop('保障有两层：\n· 资金有第三方监管\n· 签电子合同', /第三方监管/, '付款只走官方支付链接。') ===
      '保障有两层：\n· 付款只走官方支付链接。\n· 签电子合同',
  );
  ...
  check(
    'D5 命中位置对得上原文',
    hits.length === 2 && t.slice(hits[0].at, hits[0].end).startsWith('３８０００') && t.slice(hits[1].at, hits[1].end).includes('33,333'),
    JSON.stringify(hits),
  );
...
// 支持开专票、资金第三方监管、对公账户、帮订机票、电话联系。对不上就删那一句（服务承诺换成「由顾问确认」）
{
  ...
  for (const t of [
    '孩子和大人同价，我们没有单独的儿童价，按一位出行算。',
    '我们不区分平日价和节假日价，只看出发月份。',
    '儿童价是没有的，孩子也按一位算。',
  ]) {
  ...
  // R4 拿节日给价格找理由
  for (const t of [
    '价格便宜是因为 4 月 5 日避开了一些节日出行高峰。',
    '4 月已经过了春节旺季，酒店和地接价格都回落了。',
    '这个点走刚好避开十一高峰，性价比比国庆期间高不少。',
  ]) {
  ...
  check(
    'R4 出发月恰好在最佳季的「国庆档期价格上浮 10%」不删',
    !cut('· 国庆档期价格上浮 10%', await quoted('r-tibet-mid', 2, '2026-10-03')),
  );
  ...
  check(
    'R5 下过单、lastQuote 清掉了：按订单人数折算总价',
    !cut(
      '2 人总价 58960 元（人均 29480，仍在您 3 万预算内）。',
      await (async () => {
  ...
  check(
    'R5 budgetVerdict：超预算带每人差额',
    v?.fields.withinBudget === false && v.fields.overBudgetPerPerson === 1680 && v.gap === 1680,
    JSON.stringify(v),
  );
  ...
  check(
    'R5 没说每人还是总共：不判在不在预算内',
    budgetVerdict(unclear, { perPerson: 15800, total: 31600, travelers: 2 })?.fields.withinBudget === undefined,
  );
  check(
    'R5 说的是下限：不判',
    budgetVerdict(blank({ messages: [said('预算至少每人3万')] }), { perPerson: 45800, total: 91600, travelers: 2 }) === undefined,
  );
  // R6 做不到的服务：换成「由顾问确认」
  ...
  check(
    ...
    svc('可以的，我们支持开公司抬头的增值税专用发票。其他还有想了解的吗？') ===
      '发票（含专票）怎么开，由顾问跟您确认。其他还有想了解的吗？',
    svc('可以的，我们支持开公司抬头的增值税专用发票。其他还有想了解的吗？'),
  );
  check(
    'R6 资金监管 / 对公账户 / 先看材料再付款',
    !/第三方监管|对公|没问题再付款/.test(
      svc('· 下单走我们官方支付链接，资金有第三方监管\n· 付款是对公账户\n· 您下单后我们可以先把这些材料发您确认，没问题再付款。'),
    ),
  );
  check(
    'R6 机票代订、含机票报价',
    !/帮您询价|含机票的完整报价/.test(svc('机票我可以帮您询价一起订。\n需要我把含机票的完整报价算一份吗？')),
  );
  check('R6 档期余位', svc('10月30日出发档期没问题。') === '具体档期和余位由顾问跟您确认。');
  check(
    'R6 电话联系 → 微信',
    svc('好的，已经为您转接资深顾问啦，他会尽快电话联系您～') === '好的，已经为您转接资深顾问啦，他会尽快在微信上联系您～',
  );
  check('R6 给您打电话 → 微信', svc('顾问稍后给您打电话。') === '顾问稍后在微信上联系您。');
  for (const t of [
    '下单后签电子合同，付款只走我们发给您的官方支付链接。',
    '发票的事由顾问跟您确认。',
    '机票和签证我可以让顾问帮您对接办理。',
    '这条费用不含国际机票和签证。',
    '合同发票齐全，您放心。',
...
// ---------------- 复核（2026-09-25）：差额只在比价的小句里认 / 如实的话不误删 / 删分点带走续行 / 预算断言只删小句 ----------------
{
  ...
  for (const t of ['总价再加 5,000 元就能升级到海景套房。', '签证加机票每人大概 4,700 元。'])
    check(`X1 两次报价之差的约数不给别的费用放行：${t}`, blocked(t, sanya2));
  ...
  for (const t of ['以上是不含机票的价格，往返机票需要您自理。', '这个报价是不含机票的价格。'])
    check(`X2 不动：${t}`, kept(t), dropUnbackedClaims(t, blank()).text);
  ...
  for (const t of [
    '水屋房间面积有限，一家四口建议订两间。',
    '3 岁以下小朋友免费入园，以乐园规定为准。',
    '因为您只有 5 天假期，推荐这条 5 日的线路，价格也合适。',
    '11 月中旬出发正好错开国庆高峰，人少景美，性价比很高。',
    '您可以先看方案书确认行程，满意后再付款。',
    '儿童价格跟大人一样，都按一位出行算。',
    '小朋友价格和大人一样，按一位算。',
    '档期没问题的话，我这边就给您下单。',
  ])
    check(`X3 不动：${t}`, kept(t, mv), dropUnbackedClaims(t, mv).text);
  for (const t of ['小朋友按儿童价更划算。', '国庆档期很抢手。', '小朋友免费，团费只算大人。', '档期没问题。'])
    check(`X3 照删：${t}`, !kept(t, mv));
  ...
  const x4 = dropUnbackedClaims('10 月 5 日出发每人 29,480 元，2 位总价 58,960 元，基本在您预算内。要不要我先给您出一份方案书？', b, [
    { name: 'create_quote', args: {} },
  ]).text;
  check('X4 只删「基本在您预算内」', x4 === '10 月 5 日出发每人 29,480 元，2 位总价 58,960 元。要不要我先给您出一份方案书？', x4);
  const x4b = dropUnbackedClaims(
    '4 月 5 日出发每人 15,800 元，价格便宜是因为避开了节日高峰。',
    await (async () => {
      const s = blank();
      await executeTool('create_quote', { routeId: 'r-sanya', travelers: 2, departDate: '2027-04-05' }, s);
      return s;
    })(),
  ).text;
...
// ---------------- 第二轮验证（2026-09-25）：季节判断 / 预算判断的更多说法 / 做不到的加减 / 高反担保 / 按句删后残句 ----------------
{
  ...
  check(
    'Z1 「换到11月错峰」接在最佳季那句后面：删',
    cutIn('10月是这条线的最佳出行季，价格上浮了10%。换到11月或明年初错峰，我按日期给您实报一个价，对比着看。', yn),
  );
  ...
  check(
    'Z1 B01「换到11月中以后错开」（九寨最佳季到 11 月）删',
  ...
  check(
    'Z2 人数只在客户那句预算里：「两位的话在 3 万预算内完全可行」删',
    cutIn('我们有一条巴厘岛乌布雨林蜜月 6 日，人均 22800 起，两位的话在 3 万预算内完全可行：', a04, baliSearch),
  );
  check(
    'Z2 「预算按这条算很宽裕」删',
    cutIn('巴厘岛乌布雨林蜜月 6 日，每人 22800 起。两位的预算按这条算很宽裕，还能升房型或加天数。', a04, baliSearch, 2),
  );
  ...
  check(
    'Z4 「云南线路大多在 2000 多米，基本不用担心高反」删',
    cutIn('云南确实更安心——线路大多在 2000 多米，基本不用担心高反，蜜月也合适。', tibet),
  );
  ...
  check(
    'Z6 「出了 10 月（比如 11 月出发）就不在最佳季」：10 月是分界，不删',
    !cutIn('如果日期上灵活的话，出了 10 月（比如 11 月出发）就不在最佳季，按标准价走，会低不少。', bali10),
  );
  check(
    'Z6 同一行前面点了马尔代夫：「11月-次年4月是最佳季」按马代核，不删',
    !cutIn('马尔代夫：纯粹的海岛度假，私密性最强。11月-次年4月是最佳季，现在出发（9-10月）反而避开高峰。', bali10),
  );
  check(
    'Z6 上一行是贵州：「全程最高约 1200 米，没有高原段」不删',
    !cutIn(
      '稻城亚丁这条最高要到约 4700 米。\n· 贵州荔波小七孔·西江千户苗寨 6 日\n· 全程最高约 1200 米，没有高原段',
      shown('r-sichuan-lux', 'r-guizhou'),
    ),
  );
  ...
  check(
    'Z5 线路名和价删了、剩「这条的亮点」：残句',
    sd(
      '先说个实际情况：我们马代的线路起步是中央格兰德一价全包 5 日，人均 28,800 起，两位的话总价 5.7 万左右。\n\n这条的亮点：\n· 一价全包\n· 出别墅就能下水',
      '这条的亮点：\n· 一价全包\n· 出别墅就能下水',
    ),
  );
  check(
    'Z5 「按 4 人重新报价」却一个价都没有：残句',
    sd(
      '按 4 人重新报价：\n西安 兵马俑 5 日\n· 每人 13384 元\n· 合计 53536 元\n报价为起价。',
      '按 4 人重新报价：\n西安 兵马俑 5 日\n报价为起价。',
    ),
...
// ---------------- 第三轮复核（2026-09-25）：季节小句不借前面的月份 / 千分位不断小句 / 放开预算 / 条件句的「预算够」/ 残句只看线路名 ----------------
{
  ...
  check(
    'Q1 「改到 11 月出发，就是标准价」照删',
    out('如果改到 11 月出发，就是标准价 16,800 元。', yn) === '',
    out('如果改到 11 月出发，就是标准价 16,800 元。', yn),
  );
  check(
    'Q1 「改到 12 月出发，就不在最佳季」照留（12 月不在最佳季）',
  ...
  check(
    'Q3 「预算不是问题」之后不再拿「一共3万」比',
    budgetCap(blank({ messages: [said('我们两个人 预算一共3万 想去巴厘岛'), said('算了 预算不是问题 想住好一点的 马尔代夫有啥')] })) ===
      undefined,
  );
  check(
    'Q3 「钱不是问题 预算不设上限」',
    budgetCap(blank({ messages: [said('两个人预算一共4万 想去云南'), said('钱不是问题 预算不设上限 看看最好的')] })) === undefined,
  );
  check(
    'Q3 放开之后又说了数：按新说的',
  ...
  check(
    'Q3 同一句里放开又给了数（「预算不是问题 每人5万以内都行」）：按这个数',
  ...
  for (const [t, s] of [
    ...
    check(`Q4 不是在说在预算内：不删 ${t}`, out(t, s, bali, 2) === t, out(t, s, bali, 2));
  check(
    'Q4 真在断言的「预算很宽裕」照删（两位一共 3 万、巴厘岛两位 45,600）',
    out('人均 22,800 起，两位的预算按这条算很宽裕。', blank({ messages: [said('两个人预算一共3万左右')] }), bali, 2) === '人均 22,800 起。',
  );

  // Q5 残句：「这条 / 它」只在删掉的那部分点过线路名、剩下的一条都没点时才算没了指代
  check(
    'Q5 只删了价、原文本来就用「这条线」指着前文：不算残句',
    !strandedAfterDrop(
      '这条线住的是古城精品客栈，每人 16,800 元起。行程节奏很松，适合带爸妈。您几位出行？',
      '这条线住的是古城精品客栈。行程节奏很松，适合带爸妈。您几位出行？',
    ),
  );
  check(
    'Q5 线路名还在（「推荐北欧极光 8 日。它的玻璃屋…」）：不算残句',
    !strandedAfterDrop('推荐北欧极光 8 日，人均 46,800 元起。它的玻璃屋可以躺着看极光。', '推荐北欧极光 8 日。它的玻璃屋可以躺着看极光。'),
  );
  check(
    'Q5 线路名跟着价一起删了、只剩「这条线路」：仍是残句',
...
// ---------------- 第四轮定点修复（2026-09-25）：季节护栏误删说对的话 / 资金托管往顾问引不换、不重复 / 「按这个数找线路」不是成交 ----------------
{
  ...
  const yn = await quotedS('r-yunnan-mid', 2, '2026-10-24');
  check(
    'S1 回复里还有别的线在场、这句没点名：对得上其中一条就不删',
    dropped(
      '再给您推一条：三亚亲子奢华度假 5 日，人均 15,800 起。\n\n亚特兰蒂斯海景房，孩子玩水方便。\n\n12 月是最佳季，价格上浮 10%。',
      yn,
    ).length === 0,
  );
  // 上一轮必须删的照删
  check(
    'S1 A09「换到不在最佳季的月份（比如11月底、12月）」照删（11 月在丽江大理最佳季，12 月在梅里线最佳季，跟在场的两条都对不上）',
    dropped(
      ...
      await quotedS('r-yunnan-mid', 4, '2026-10-24'),
    ).length === 1,
  );
  ...
  check(
    'S2 guard-13「资金监管这块我确认不了，由顾问跟您确认」不换',
    dropUnbackedClaims('资金监管这块我确认不了，由顾问跟您确认。', plain).text === '资金监管这块我确认不了，由顾问跟您确认。',
  );
  check(
    'S2 「以合同约定为准」不换',
    dropUnbackedClaims('资金托管的具体安排以合同约定为准。', plain).text === '资金托管的具体安排以合同约定为准。',
  );
  ...
  for (const t of ['我按这个数帮您找匹配的线路。', '您说个数，我按这个数给您推荐。', '我按这个预算帮您筛一下。'])
    check(`S3 「${t}」不是成交语境`, !closing(t));
  for (const t of ['好的，就按这个数给您锁定。', '您预算 6800，行，就按这个数来。', '好的，就按这个数帮您下单看看'])
    check(`S3 「${t}」仍是成交语境`, closing(t));
}
...
// ---------------- 第五轮复核（2026-09-25）：季节核对只认点了名的具体线路 / 顺着砍价的「就按这个数」照拦 / 资金担保顺带一句顾问照换 ----------------
{
  ...
  const search = (destination: string, ...ids: string[]) => ({
    name: 'search_routes',
    args: { destination },
    result: JSON.stringify(ids.map((id) => ({ id }))),
  });
  ...
  for (const t of [
    '行吧，那就按这个数您看行不行：每人 6,800 元。',
    '可以，就按这个数给您看看名额，每人 6,800 元。',
    '就按这个数帮您查一下能不能下单，每人 6,800 元。',
    '我按这个数帮您查一下能不能下单，每人 6,800 元。',
    '您预算 6800，行，按这个数给您安排：每人 6,800 元。',
  ])
    check(
      `R2 「${t}」拦下 6,800`,
      JSON.stringify(findUnbackedPrices(t, hq, haggle)) === '[6800]',
      JSON.stringify(findUnbackedPrices(t, hq, haggle)),
    );
  const closing = __priceGuardTest.hasClosingPrice as (s: string) => boolean;
  for (const t of ['我按这个预算帮您挑几条合适的线路。', '您说个数，我按这个预算帮您查查有哪些线路。'])
    check(`R2 「${t}」仍是按预算检索`, !closing(t));

  // R3 编的资金担保顺带一句「找顾问 / 以合同为准 / 顾问会说明」：照换。往顾问引的说法得跟资金那句在同一个小句，或资金说法本身是话题
  const plain = blank({ stage: 'recommend' });
  const REPL = '付款只走我们发给您的官方支付链接。';
  for (const t of [
    '您的款项全程第三方监管，有疑问随时找顾问。',
    '您的款项会进入第三方监管账户，行程结束才结算给地接，具体以合同为准。',
    '资金由银行第三方托管，顾问会给您详细说明。',
    '费用第三方托管，下单后跟顾问签电子合同就行。',
  ])
...
console.log(
  `PRICE-GUARD SELFTEST PASS: ${pass} 项断言全通（编价拦截 / 真实报价零误杀 / 档位话术零误杀 / 差额与中文预算 / 回复里的中文数字金额 / 酒店每晚价 / 产品库价按本会话出现过的线路核 / 报价之差与量词 / 只删那一句 / 规则词·预算·服务承诺 / 复核：差额看上下文·如实不误删·预算只删小句·续行随分点 / 第二轮：季节按 bestSeason 核·预算的其他说法·做不到的加减·高反担保·残句 / 第三轮：换日子的小句不借月份·千分位不断句·放开预算·条件句的预算够·残句看丢了哪条线 / 第四轮：说对的季节不删·资金往顾问引不换不重复·按这个数找线路不是成交 / 第五轮：季节只认点了名的具体线路·顺着砍价的就按这个数照拦·资金担保带句顾问照换）`,
);
